...
public class SalePanel extends CommonPanel {
	...
	// 按书名查询的按钮
	JButton queyrB_Button;
	// 查询按钮
	JButton queyrD_Button;
	// 查询输入的书名
	JTextField queryBook;
	// 查询输入的日期
	...
	public SalePanel(BookService bookService, SaleRecordService saleRecordService) {
		...
		this.queryBook = new JTextField(20);
		queryBox.add(this.queryBook);

		queryBox.add(Box.createHorizontalStrut(30));
		this.queyrB_Button = new JButton("查询");
		queryBox.add(this.queyrB_Button);
		...
		queryBox.add(Box.createHorizontalStrut(30));
		this.queyrD_Button = new JButton("查询");
		queryBox.add(this.queyrD_Button);
		queryPanel.add(queryBox);
		this.add(queryPanel);
		// 列表为添加界面
	...
	private void initListeners() {
		...
		// 按日期查询
		this.queyrD_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				query();
			}
		});
		// 按书名查询
		this.queyrB_Button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				query2();
			}
		});
	...
	private void query2() {
		String BookName = this.queryBook.getText();
		if(BookName.equals("")){
			showWarn("请输入书名！");
			return;
		}
		// 重新执行查询
		Vector<SaleRecord> records = (Vector<SaleRecord>) saleRecordService.getAllBook(BookName);
		Vector<Vector> datas = changeDatas(records);
		setDatas(datas);
		// 刷新列表
		refreshTable();
	...
	private void sale() {
		...
		r.setBookNames(this.bookComboBox.getSelectedItem().toString());
		if(saleRecordService.saveRecord(r))
			;
		else
			showWarn("超出库存！");
		// 重新读取数据
	...
	private void countTotalPrice() {
		double totalPrice = 0;
		for (BookSaleRecord r : this.bookSaleRecordDatas) {
			totalPrice += (Integer.valueOf(r.getTRADE_SUM()) * Double.valueOf(r.getBook().getBOOK_PRICE()));
		}
