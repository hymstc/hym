...
public class RepertoryPanel extends CommonPanel {
	...
	public RepertoryPanel(InRecordService inRecordService, BookService bookService) {
		...
		//按书名查询的按钮
		queryBox.add(Box.createHorizontalStrut(30));
		this.queyrBookButton = new JButton("查询");
		queryBox.add(this.queyrBookButton);
		queryBox.add(Box.createHorizontalStrut(30));
	...
	private void initListeners() {
		...
		//按书名查询
		this.queyrBookButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Bookquery();
			}
		});
	...
	private void Bookquery() {
		...
		if (this.queryBook.getText().equals("")) {
			showWarn("请输入书名"); 
			return;
		}
		//重新执行查询
		Vector<InRecord> records = (Vector<InRecord>)inRecordService.getAllBook(bookName);
	...
	private void dataQuery() {
		...
		//重新执行查询
		Vector<InRecord> records = (Vector<InRecord>)inRecordService.getAll(d);
		Vector<Vector> datas = changeDatas(records);
		setDatas(datas);
		//刷新列表
		refreshTable();
