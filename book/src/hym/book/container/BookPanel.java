...
import hym.book.Tool.ImageUtil;
import hym.book.Tool.UploadException;
...
public class BookPanel extends CommonPanel {
	...
	private final static String defalutPic = "upload/nopic.jpg";
	...
	public void initImage() {
		this.currentImage = new ImageIcon(defalutPic);
		this.currentImagePath = defalutPic;
		refreshImage();
	}

	public BookPanel(BookService bookService, TypeService typeService, ConcernService concernService) {
		...
		queryNameTextField = new JTextField(20);
		queryBox.add(queryNameTextField);
		queryBox.add(Box.createHorizontalStrut(30));
	...
	public void upload(File selectFile) {
		try {
			...
			// 生成缩略图
			File file = ImageUtil.makeImage(selectFile, smallFilePath, "jpg");
			// 设置界面显示的图片对象
	...
	private Book getBook() {
		...
		return new Book(null, bookName, intro, price, type.getID(), concern.getID(), String.valueOf(0),
				this.currentImagePath, author);
	}
